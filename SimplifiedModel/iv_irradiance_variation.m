...
% Loop over each irradiance level to plot I-V curves
for G = irradiance_levels
    % Calculate Photocurrent Ipv based on KI and irradiance G
    ...
    % Plot the I-V Curve for each irradiance level
    plot(Vpv, Ipv_curve, 'DisplayName', ['G = ', num2str(G), ' W/m^2'], 'LineWidth', 1.5);
end

% Add plot details
title('I-V Curves of Renesola JC260S PV module for Different Irradiance Levels');
xlabel('Voltage (V)');
ylabel('Current (A)');
xlim([0 Voc]);
ylim([0 Isc + 1]);
...
disp('I-V characteristics computed for Renesola JC260S PV module  across different irradiance levels.');
